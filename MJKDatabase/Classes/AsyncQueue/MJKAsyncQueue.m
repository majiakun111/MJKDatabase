...
@implementation MJKAsyncQueue

+ (instancetype)sharedInstance {
    static MJKAsyncQueue *instance = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        if (nil == instance) {
            instance = [[MJKAsyncQueue alloc] init];
        }
    });
    ...
    void (^warpperBlock)(void) = ^{
        ...
        switch (transactionType) {
            case  TransactionForDeferredType: {
                [[MJKDatabaseDAO sharedInstance] beginDeferredTransaction];
                break;
            }
            case TransactionForImmediateType: {
                [[MJKDatabaseDAO sharedInstance] beginImmediateTransaction];
                break;
            }
            case TransactionForExclusiveType: {
                [[MJKDatabaseDAO sharedInstance] beginExclusiveTransaction];
                break;
            }
            default:
                break;
        ...
        if (shouldRollback) {
            [[MJKDatabaseDAO sharedInstance] rollback];
        }
        else {
            [[MJKDatabaseDAO sharedInstance] commit];
        }
    };
