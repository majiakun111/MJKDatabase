...
@interface MJKStatement : NSObject
...
@interface MJKDatabase ()

@property(nonatomic, copy) NSString *databasePath;
@property(nonatomic, strong) NSMutableDictionary<NSString*,  NSMutableSet<MJKStatement *>*> *cachedStatements;

@end

@implementation MJKDatabase
...
- (BOOL)executeUpdate:(NSString*)sql {
    ...
    do {
        ...
        if (!statement && self.shouldCacheStatements) {
            statement = [[MJKStatement alloc] init];
...
- (void)clearCachedStatements {
    [self.cachedStatements enumerateKeysAndObjectsUsingBlock:^(NSString * _Nonnull key, NSMutableSet<MJKStatement *> * _Nonnull statements, BOOL * _Nonnull stop) {
        [statements enumerateObjectsUsingBlock:^(MJKStatement * _Nonnull statement, BOOL * _Nonnull stop) {
            [statement close];
        }];
    }];
...
- (MJKStatement*)cachedStatementForQuery:(NSString *)query {
    __block MJKStatement *statement = nil;
    NSMutableSet<MJKStatement *> *statements = [self.cachedStatements objectForKey:query];
    [statements enumerateObjectsUsingBlock:^(MJKStatement * _Nonnull obj, BOOL * _Nonnull stop) {
        if ([obj inUse]) {
            return;
        }
...
- (void)setCachedStatement:(MJKStatement *)statement forQuery:(NSString *)query {
