...
*2. 若属性是NSArray 可以存 MJKRecord (但必须要实现mj_objectClassInArray 指定NSArray存的是那个Class, 支持嵌套), 也可以存 NSNumber, NSString, NSArray, NSDictionary
*3. NSDictionary(不能包含 MJKRecord对象)
*4. 支持迁移
*5. 对于column add and delete, index add and delete支持自动迁移, 只需要更改DatabaseVersion即可和配置
*/

@interface MJKRecord : NSObject
