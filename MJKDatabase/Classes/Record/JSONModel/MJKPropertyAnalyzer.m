...
@implementation MJKPropertyInfo
...
@implementation MJKPropertyAnalyzer

+ (NSArray<MJKPropertyInfo *> *)getPropertyInfoListForClass:(Class)clazz {
    NSMutableArray<MJKPropertyInfo *> *propertyInfoList = objc_getAssociatedObject(clazz, &PropertyInfoListAssociatedKey);
    if (propertyInfoList) {
    ...
    for (unsigned int index = 0; index < propertyCount; ++index) {
        ...
        MJKPropertyInfo *propertyInfo = [[MJKPropertyInfo alloc] initWithProperty:property];
...
+ (NSArray *)getColumnsValueListWithPropertyList:(NSArray<NSString *> *)propertyList forObject:(NSObject *)object {
    ...
    for (NSString *propertyName in propertyList) {
        ...
        if ([value isKindOfClass:[NSArray class]]) {
            [propertyValueList addObject:[MJKArrayConverter getValuesWithArrayValue:value propertyName:propertyName forObject:object]];
