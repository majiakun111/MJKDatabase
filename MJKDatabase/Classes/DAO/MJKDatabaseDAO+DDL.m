...
- (BOOL)buildTable:(NSString *)tableName constraints:(NSDictionary *)constraints indexes:(NSDictionary *)indexes forClass:(Class)clazz;
...
- (BOOL)buildTable:(NSString *)tableName constraints:(NSDictionary *)constraints indexes:(NSDictionary *)indexes forClass:(Class)clazz {
    ...
    NSArray *propertyInfoList = [MJKPropertyAnalyzer getPropertyInfoListForClass:clazz];
    ...
    for (NSInteger i = 0; i < count; i++) {
        ...
        MJKPropertyInfo *propertyInfo = propertyInfoList[i];
    ...
    //create table
    BOOL result = [[MJKDatabaseDAO sharedInstance] executeUpdate:sql];
    if (result) {
    ...
    for (NSString *columnName in indexes) {
        ...
        [[MJKDatabaseDAO sharedInstance] createIndex:indexInfo[INDEX_NAME] onColumn:columnName isUnique:[indexInfo[IS_UNIQUE] boolValue] forTable:tableName];
    }
...
@implementation MJKDatabaseDAO (DDL)
...
- (BOOL)createTable:(NSString *)tableName constraints:(NSDictionary<NSString* , NSString *> *)constraints indexes:(NSDictionary<NSString*, NSDictionary*> *)indexes forClass:(Class)clazz {
    return [[TableBuilder sharedInstance] buildTable:tableName constraints:constraints indexes:indexes forClass:clazz];
}
